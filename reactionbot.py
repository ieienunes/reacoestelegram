...
# Adicionar um formatador que pode lidar com emojis
class EmojiFormatter(logging.Formatter):
    def format(self, record):
        record.msg = emoji.demojize(record.msg)
        return super().format(record)

formatter = EmojiFormatter('%(asctime)s - %(levelname)s - %(message)s')
logger.handlers[0].setFormatter(formatter)

# Usar a função de formatação de emojis no logger
logger.info = lambda msg, *args: logger.info(msg, *args)
logger.warning = lambda msg, *args: logger.warning(msg, *args)
logger.error = lambda msg, *args: logger.error(msg, *args)
logger.debug = lambda msg, *args: logger.debug(msg, *args)
...
for logger_name in loggers:
    ...
    logger.warning('Inicie o bot de reação.')
...
# Lista de emojis que você deseja usar para reações
EMOJIS = ["👍", "❤️", "👏", "🔥", "😍", "🎉"]
...
async def send_reaction(client: Client, message: types.Message) -> None:
    ...
    try:
        ...
        error.warning(f'{emoji} - reação inválida')
    except UserDeactivatedBan:
        error.warning(f'Session Banida - {client.name}')
    except Exception:
        error.warning(traceback.format_exc())
    else:
        info.info(f'Session {client.name} Enviado - {emoji}')
...
async def main():
    ...
    for app, config_dict, session_file_path in apps:
        ...
        try:
            ...
            error.warning(f'Session Banida - {app.name}')
            apps.remove((app, config_dict, session_file_path))
            continue
        except Exception:
            apps.remove((app, config_dict, session_file_path))
        ...
        info.info(f'Session Iniciada - {app.name}')
    ...
    info.warning('Todas Session Iniciadas!')
    await idle()

    for app, _, _ in apps:
        try:
            info.warning(f'Parado - {app.name}')
            await app.stop()
...
def start():
    ...
    try:
        ...
        error.warning(f'Aguardando {TRY_AGAIN_SLEEP} sec. before restarting the program...')
